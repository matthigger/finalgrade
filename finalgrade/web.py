...
def default_yaml():
    ...
    Ninety of its hundred-odd lines are a lesson in yaml: how to indent, and
    a worked example of every section.  That is written for somebody editing
    the file by hand, which is not what happens here -- every section has a
    widget, and the reader of this page saves the file rather than reads it.
    Left in, they bury the thirty lines that actually grade.

    What the seeded block says survives, because that part is about this
    course rather than about yaml: see seed_policy.  So does every comment
    in a policy the reader brings with them -- those are theirs, and the
    widget edits are built to preserve them.
    """
