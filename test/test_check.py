""" the report that says what a config would do, before it does it
...
class TestMapping:
    def test_category_catches_assignments(self, f_scope_std):
        report = build_report(
            Config(cat_weight_dict={'hw': 50, 'quiz': 50}), str(f_scope_std))
    ...
    def test_weight_is_normalized(self, f_scope_std):
        ...
        report = build_report(
            Config(cat_weight_dict={'hw': 3, 'quiz': 1}), str(f_scope_std))
    ...
    def test_category_matching_nothing_is_an_error(self, f_scope_std):
        report = build_report(
            Config(cat_weight_dict={'hw': 50, 'exam': 50}), str(f_scope_std))
    ...
    def test_every_problem_at_once(self, f_scope_std):
        ...
        assert any('exam' in s for s in report.error_list)
        assert any('quiz1' in s for s in report.warn_list)

    def test_assignment_in_no_category_is_a_warning(self, f_scope_std):
        report = build_report(
            Config(cat_weight_dict={'hw': 100}), str(f_scope_std))
    ...
    def test_assignment_in_two_categories_is_a_warning(self, f_scope_std):
        """ 'hw' and 'w' both catch every hw: substring matching's own trap """
        report = build_report(
            Config(cat_weight_dict={'hw': 50, 'w': 50}), str(f_scope_std))

        ass_dict = {ass.name: ass.cat_list for ass in report.ass_list}
        assert ass_dict['hw1'] == ['hw', 'w']
        assert any('multiple categories' in s for s in report.warn_list)

    def test_no_category_weights_by_point(self, f_scope_std):
        report = build_report(Config(), str(f_scope_std))
...
class TestSubmitted:
    ...
    def test_student_count_follows_email_list(self, f_scope_std):
        report = build_report(
            Config(email_list=['alice@u.edu']), str(f_scope_std))
...
class TestExcluded:
    def test_excluded_says_which_rule_dropped_it(self, f_scope_std):
        report = build_report(
            Config(remove_list=['quiz']), str(f_scope_std))
    ...
    def test_completion_threshold_names_itself(self, f_scope_std):
        report = build_report(
            Config(exclude_complete_thresh=.9), str(f_scope_std))
...
class TestBadInput:
    def test_unreadable_csv_is_an_error_not_a_traceback(self, tmp_path):
        ...
        report = build_report(Config(), str(f_csv))
    ...
    def test_config_error_is_collected(self, f_scope_std):
        ...
        report = build_report(
            Config(sub_dict={'nope': ['also_nope']}), str(f_scope_std))
...
class TestRender:
    def test_shows_the_split(self, f_scope_std):
        ...
        assert 'config looks usable' in text
    ...
    def test_says_when_grading_would_stop(self, f_scope_std):
        text = render(build_report(
            Config(cat_weight_dict={'exam': 100}), str(f_scope_std)))

        assert 'config has an error' in text

    def test_late_penalty_is_legible(self, f_scope_std):
        text = render(build_report(
            Config(cat_weight_dict={'hw': 100},
...
class TestCli:
    def test_check_exits_nonzero_on_error(self, f_scope_std, write_config,
        ...
        args = parser.parse_args(
            ['check', str(f_scope_std), '--config', str(f_config)])
    ...
    def test_check_is_quiet_and_clean_when_ok(self, f_scope_std, write_config,
        ...
        assert 'config looks usable' in capsys.readouterr().out
