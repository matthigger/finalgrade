""" an email the policy names but the gradebook doesn't have
...
class TestTypoIsAnError:
    def test_waive(self, f_scope_std):
        policy = Policy(waive_dict={'alicce@u.edu': 'hw1'})

        with pytest.raises(PolicyError, match='alicce'):
            policy(str(f_scope_std))

    def test_waive_late(self, f_scope_std):
        policy = Policy(late_waive_dict={'nobody@elsewhere.edu': 'hw1'})

        with pytest.raises(PolicyError, match='nobody'):
            policy(str(f_scope_std))
    ...
    def test_suggests_the_student_meant(self, f_scope_std):
        ...
        with pytest.raises(PolicyError, match='alice@u.edu'):
            policy(str(f_scope_std))

    def test_reports_every_bad_email_at_once(self, f_scope_std):
        policy = Policy(waive_dict={'nope1@u.edu': 'hw1',
                                    'nope2@u.edu': 'hw2'})
...
class TestStillAccepted:
    def test_matching_email_prefix(self, f_scope_std):
        ...
        policy = Policy(waive_dict={'alice@husky.nu.edu': 'hw1'})
    ...
    def test_student_pruned_by_email_list(self, f_scope_std):
        ...
        policy = Policy(email_list=['alice@u.edu', 'bob@u.edu'],
    ...
    def test_email_list_itself_only_warns(self, f_scope_std):
        ...
        """
        policy = Policy(email_list=['alice@u.edu', 'never@u.edu'])
...
class TestNoInventedStudent:
    ...
    def test_pruned_student_waiver_leaves_the_roster_alone(self, f_scope_std):
        ...
        policy = Policy(email_list=['alice@u.edu'],
                        waive_dict={'carol@u.edu': 'hw1'})
...
class TestCheckReportsIt:
    def test_check_shows_the_bad_email(self, f_scope_std):
        ...
        report = build_report(Policy(waive_dict={'alicce@u.edu': 'hw1'}),
                              str(f_scope_std))
