"""End-to-end tests through the Policy / CLI boundary.

These are deliberately written against the *public* boundary (a scope csv and
a policy in, a grade dataframe out) rather than against Gradebook internals,
...
class TestPipelineGolden:
    ...
    def test_category_means(self, f_scope_std):
        _, df = Policy(cat_weight_dict={'hw': 1, 'quiz': 1})(f_scope_std)
    ...
    def test_unequal_category_weights(self, f_scope_std):
        _, df = Policy(cat_weight_dict={'hw': 3, 'quiz': 1})(f_scope_std)
...
class TestFeatureInteractions:
    ...
    def test_substitute_then_exclude(self, tmp_path):
        ...
        with pytest.warns(UserWarning, match='prefixes'):
            _, df = Policy(sub_dict={'quiz1': ['quiz1v2']},
    ...
    def test_drop_low_respects_point_weights(self, tmp_path):
        ...
        _, df = Policy(cat_weight_dict={'hw': 1})(f)
        # (0*10 + .5*100) / 110
        assert df.loc['a@u.edu', 'mean_hw'] == pytest.approx(50 / 110)

        _, df = Policy(cat_weight_dict={'hw': 1}, cat_drop_dict={'hw': 1})(f)
    ...
    def test_extra_metadata_column(self, tmp_path):
        ...
        f = write_scope(
            ...
            meta_header=['First Name', 'Last Name', 'SID', 'Email',
                         'Sections', 'CRN'])
        _, df = Policy(cat_weight_dict={'hw': 1, 'quiz': 1})(f)
...
class TestConfigValidationEndToEnd:
    """ a bad policy must fail loudly, never produce plausible wrong grades """
...
class TestDeterminism:
    def test_output_row_order_is_stable(self, tmp_path):
        ...
        script = (
            ...
            'from finalgrade.policy import Policy;'
            f'_, df = Policy(email_list={[s["email"] for s in STUDENT_STD]!r})'
            f'({str(f_scope)!r});'
    ...
    def test_row_order_follows_input(self, f_scope_std):
        ...
        _, df = Policy(email_list=['carol@u.edu', 'alice@u.edu'])(f_scope_std)
...
class TestCLI:
    ...
    def test_grade_writes_expected_values(self, tmp_path, f_scope_std):
        ...
        main(parser.parse_args(
            ['grade', str(f_scope_std), '--policy', f_cfg, '-q']))
...
""")
        main(parser.parse_args(
            ['grade', str(f_scope_std), '--policy', f_cfg,
             '--late_csv', 'late.csv', '-q']))
    ...
    def test_per_student_files_are_unique(self, tmp_path):
        ...
        main(parser.parse_args(
            ['grade', str(f), '--policy', f_cfg, '--per_student', '-q']))
...
class TestExporters:
    def test_canvas_cli_end_to_end(self, tmp_path):
        ...
        f_scope = write_scope(
            ...
            meta_header=['First Name', 'Last Name', 'SID', 'Email',
                         'section_name'])
        f_cfg = tmp_path / 'policy.yaml'
        f_cfg.write_text(CFG_BASE)
        main(parser.parse_args(
            ['grade', str(f_scope), '--policy', str(f_cfg), '-q']))
    ...
    def test_canvas_merge_does_not_mutate_arguments(self, tmp_path):
        ...
        f_scope = write_scope(tmp_path / 'scope.csv', ASSIGN_STD, STUDENT_STD)
        _, df_grade = Policy(cat_weight_dict={'hw': 1, 'quiz': 1})(f_scope)
    ...
    def test_banner_cli_without_crn(self, tmp_path, f_scope_std):
        ...
        f_cfg.write_text(CFG_BASE)
        main(parser.parse_args(
            ['grade', str(f_scope_std), '--policy', str(f_cfg), '-q']))
        main(parser.parse_args(
