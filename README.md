...
- **Students** — search by name to see every score they have, grouped by category. **Clicking a score waives it**, and clicking it again puts it back. A score that was never handed in reads *none*, a submitted zero reads *0%*, and one you haven't set yet reads *not set* — three things that are all zero points and mean entirely different things. **Dragging one score onto another** gives that student the better of the two — the makeup only they sat, without inventing a policy-wide rule for it. Each one appears as a chip with an × on it, since a drag is easy to make by accident and otherwise leaves no trace on the score it changed. Below the scores, every assignment a late penalty could act on appears again with its late days and the exact time (`2d = 1d 23h`), where a click forgives the penalty — before it is incurred as readily as after.
